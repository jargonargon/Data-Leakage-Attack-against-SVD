...
for p in range(1, 9):
    ...
    CorrSummary = np.loadtxt(
        f"data_save/evaluation/4000x9-mimic-random/{p}-columns/RMSESummary.txt"
    )
    # if p != 1:
    #     CorrSummary = np.mean(CorrSummary)
...
ax.legend(frameon=False, borderaxespad=0, labelspacing=0.1, loc='lower right')
ax.set_xlabel(r"$X_{\rho}^{O}$ ($|\mathcal{F}_{\rho}|$) 内の特徴数")
ax.set_ylabel("RMSE スコア")
ax.set_ylim(-0.1, 1.6)
ax.set_xlim(0.9, 8.1)
...
plt.savefig("data_save/numFeaturesAndRMSE.pdf")
plt.show()
